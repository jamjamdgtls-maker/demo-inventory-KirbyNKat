...
export default function Sizes() {
  ...
  const handleOpen = (size?: Size) => {
    if (size) { setEditing(size); setFormData({ name: size.name, sortOrder: size.sortOrder, isActive: size.isActive }); }
    else { setEditing(null); setFormData({ name: '', sortOrder: sizes.length, isActive: true }); }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editing) { await updateDoc(doc(db, COLLECTIONS.SIZES, editing.id), { ...formData, updatedAt: serverTimestamp() }); }
      else { await addDoc(collection(db, COLLECTIONS.SIZES), { ...formData, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }); }
      toast({ title: 'Success', description: `Size ${editing ? 'updated' : 'created'}` });
  ...
  return (
    <MainLayout title="Sizes">
      <div className="space-y-4">
        <div className="flex justify-end"><Button onClick={() => handleOpen()}><Plus className="h-4 w-4 mr-2" />Add Size</Button></div>
        <Card>
          <Table>
            ...
            <TableBody>
              {sizes.length === 0 ? <TableRow><TableCell colSpan={4} className="text-center py-8"><Ruler className="h-8 w-8 mx-auto text-muted-foreground" /><p className="text-muted-foreground mt-2">No sizes</p></TableCell></TableRow> :
                sizes.map(s => <TableRow key={s.id}><TableCell className="font-medium">{s.name}</TableCell><TableCell>{s.sortOrder}</TableCell><TableCell><Badge variant={s.isActive ? 'default' : 'secondary'}>{s.isActive ? 'Active' : 'Inactive'}</Badge></TableCell><TableCell><Button variant="ghost" size="icon" onClick={() => handleOpen(s)}><Edit className="h-4 w-4" /></Button></TableCell></TableRow>)}
            </TableBody>
          </Table>
        </Card>
      </div>
